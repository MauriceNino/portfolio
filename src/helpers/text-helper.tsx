...
export class TextHelper {
  ...
  private static getLineLength(el: JSX.Element): number {
    ...
    if(typeof children === 'string') {
      ...
      return TextHelper.partitionArray(children as JSX.Element[], (e) => e.type === 'br')
        .map(el => 
  ...
  public static centered(elements: JSX.Element[], parentVCells: number, parentHCells: number): JSX.Element[] {
    const allElementsHeight = elements.map(el => TextHelper.getLineHeight(el)).reduce((a, b) => a + b, 0);
    let reachedHeight = 0;
